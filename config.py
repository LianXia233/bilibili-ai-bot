...
_DEFAULTS = {
    ...
    # 永久记忆注入提示词的**字符预算**（只约束「锦上添花」类规则）。
    #
    # 永久记忆是「人格/行为规则」层，按条数上限（40 条）管理并不够 —— 条数不约束
    # 单条长度。实测线上 14 条合计 9375 字符，其中表情包池清单与表情包细则占了近
    # 一半；再加上人格设定（persona）本身就有 7000+ 字符，整个提示词会膨胀到
    # 18000 字符，而用户真正说的那句话只有一行、位于提示词最末尾 —— 模型的注意力
    # 被前置的巨量规则吃掉，表现就是「回复与来信不匹配」「复读用户原话」「答非所问」。
    #
    # 重要：本预算**不约束**身份/人格/禁止/安全类规则 —— 那些是人格底线，
    # 一律全量注入（详见 ai.py 的 _rule_tier 与 _build_permanent_block）。
    # 这里调的只是「状态类 + 表情包类」细则的额度：
    #   调大：能多注入几条状态/表情包细则。
    #   调小：正文更聚焦当前这句话；调到 0 则只注入身份与禁令。
    # 表情包**池清单**不走本预算，它只以「条数 + 命名样例」的摘要在注入。
