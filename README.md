...
- 列表为**折叠式**：已配置的条目默认只显示一行摘要，点「编辑」才展开四个字段；
  未配置的空位折叠成一行占位。至少 5 个位置，可继续新增（上限 20 条）
- 点「启用」会自动先把当前编辑内容落盘，避免「填好模型 ID 直接点启用」时改动丢失
- 对应配置项：`CHAT_MODEL_POOL`（数组）、`CHAT_MODEL_ACTIVE`（下标，`-1` = 不用池）

#### 永久记忆与记忆清空

永久记忆保存 Bot 的**人格与行为规则**，只支持在面板「💎 永久记忆」里手动添加，
Bot 不会自动写入。上限 40 条，注入提示词时不截断。**条数之外还有字符预算**
（见下节），避免单条写得太长把提示词挤爆。
...
永久记忆按**条数**上限管理，但条数不约束单条长度。实测曾出现「14 条 = 9375 字符、
占整段 prompt 的 52%」，把用户当轮那句话挤到末尾（占 0.06%），表现为复读、答非所问、
看起来像「不同会话串台」。
...
| 3 | 表情包池相关 | 受预算约束，超预算整层跳过 |

核心原则：**tier 0/1 是人格底线，宁可挤爆预算也必须全量注入**；字符预算只用来约束
tier 2/3 这类「锦上添花」的内容。同层内按写入时间**新的先保留**，最后按原始顺序还原。

表情包池会自动摘要化（`【可用表情包素材（索引摘要）】`：共约 N 条 + 每段前 6 个样例 +
使用约束），单条实测压缩率约 89% —— 池子只需要知道「有哪些素材」，不必把每个名字念一遍。

| 配置项 | 默认 | 范围 | 说明 |
|--------|:----:|:----:|------|
| `PERMANENT_MEMORY_CHAR_BUDGET` | `2500` | 0-100000 | tier 2/3 的字符预算；**不约束**身份 / 人格 / 禁止 / 安全类规则 |

把预算调小只会裁掉 tier 2/3，人格与底线规则始终在位。

#### 临时记忆与定时清空

面板「🧠 记忆管理」分为**临时记忆**与**长期记忆**两个页签：

| 页签 | 包含 | 自动清理 |
|------|------|:--------:|
| 临时记忆 | 对话记忆（含压缩摘要）、用户档案 | 参与 |
| 长期记忆 | 永久记忆、好感度、性格演化、视频缓存、当日心情 | 不参与 |

分界依据是「清掉之后会失去什么」：临时记忆清掉只是让 Bot 忘掉聊过什么；
长期记忆（尤其永久记忆与性格演化）清掉等于回滚人格，所以定时任务永不触碰。

「⏳ 临时记忆」页提供：

- **一键清空临时记忆**，清空前每个文件自动备份（`.panelclear-时间戳.bak`）
- **定时自动清空**：设开关与每天的时刻（默认 04:00），到点自动清一次
- 可设「保留最近 N 天」：填 `0` = 每次全清；填 `7` = 只丢弃 7 天前的记录
- 显示上次执行时间与结果，以及下次执行时间

| 配置项 | 默认 | 说明 |
|------|:----:|------|
| `TEMP_MEMORY_AUTO_CLEAR` | `false` | 定时清空总开关 |
| `TEMP_MEMORY_CLEAR_HOUR` | `4` | 每天几点清（0-23） |
| `TEMP_MEMORY_CLEAR_MINUTE` | `0` | 几分清（0-59） |
| `TEMP_MEMORY_KEEP_DAYS` | `0` | 保留最近 N 天，`0` = 全清 |

> 定时清空是「今天已过设定时刻且今天没清过就执行」的语义，所以进程在目标时刻之后
> 才启动时会立即补清一次 —— 这是为了保证「每天必清一次」。若把时刻设在 04:00
> 而 Bot 当天 23:00 才重启，重启后会马上清一次。
