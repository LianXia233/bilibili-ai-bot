...
| [二十](#二十临时记忆的定时清空四层陷阱) | 定时清空临时记忆 | 日期去重 / 挂载位置 / 内存态同步 / 补跑语义，四层各有陷阱 | 看状态文件 + 跨天构造用例 |
| [二十一](#二十一评论与私信串台永久记忆膨胀淹掉用户那句话) | 评论与私信「串台」、复读、答非所问 | 永久记忆只限条数不限长度，18030 字符 prompt 里用户那句话排在末尾 | 抓真实 prompt 量化各段占比；用真实模型跑用例表前后对比 |
| [二十二](#二十二复读与空承诺从宣布意图到交付成品) | 提了写诗 / 列清单却只回「好的我来帮你」 | 成品条款与 15-40 字长度约束直接冲突，后者带「像作文」否决语 | 同一条输入做提示词 A/B；对成品类回复做结构化校验而非目测 |
| [二十三](#二十三部署注意事项) | 部署注意事项 | — | — |
...
需求是「临时的记忆也需要一键清空，可以配置定时清空」。清空本身很简单，
难的是**「每天清一次」这件事在长驻进程里有四层容易写错的判定**。

### 先划定边界：什么算「临时」

| 类别 | 文件 | 自动清理 |
|------|------|:--------:|
| 对话记忆 | `data/memory.json`（含 `[记忆压缩]` 摘要） | 是 |
| 用户档案 | `data/user_profiles.json` | 是 |
| 永久记忆 | `data/permanent_memory.json` | 否 |
| 好感度 | `data/affection.json` | 否 |
| 性格演化 | `data/personality_evolution.json` | 否 |
| 视频缓存 | `data/video_memory.json` | 否 |
| 当日心情 | `data/mood.json` | 否 |

分界不是按文件大小，而是按「清掉之后会失去什么」：临时记忆清掉只是忘掉聊过什么，
长期记忆清掉等于回滚人格。清单收敛在 `config.py` 的 `TEMP_MEMORY_FILES` /
`LONG_MEMORY_FILES` 两个元组里，面板进程与 Bot 进程共用 —— 两边各写一份清单时，
最典型的故障是「面板清了两份、Bot 清了三份」，且症状是「清空了但 Bot 还记得」，
从现象几乎推不回原因。

### 陷阱一：用 `now.hour == 目标小时` 判触发

主循环是长驻进程、每分钟一轮。若判据写成小时相等，则在那一小时内的**每一轮都会命中**，
一小时内清 60 次 —— 把随后新写入的记忆也一起抹掉，用户看到的是「Bot 完全失忆」。

**正解**：状态文件 `data/temp_clear_state.json` 记 `last_clear` 日期，判据是
「今天清过没有」。一天最多执行一次，进程重启也不会重复。
...
（对比：`maybe_evolve_personality` 用的是 `now.hour == EVOLVE_HOUR`，它有一小时窗内
重复触发的隐患，只是靠 `last_evolve` 日期标记兜住了。新写的逻辑直接采用日期标记。）

### 陷阱二：挂载位置在休眠判断之后

```python
maybe_evolve_personality(memory)
memory = maybe_clear_temp_memory(memory)   # ← 必须在 continue 之前
if not is_active_time():
    time.sleep(60)
    continue
```

若放在 `continue` 之后，开启休眠时整个休眠窗内的清空都不会执行 —— 而用户恰恰最可能
把清空时间设在深夜 3、4 点，那就永远不会触发。

### 陷阱三：只改文件，不同步进程内存态

主循环里有一份常驻的 `memory` 列表（启动时 `load_memory()` 载入）。若清空只写文件，
后续 `save_memory_record` 会基于**内存里那份旧数据**追加新记录再整体写回，
表现为「刚清完几分钟，记忆又全回来了」。

**正解**：函数返回重新读盘的列表，调用方必须重新绑定：

```python
memory = maybe_clear_temp_memory(memory)
```

### 陷阱四：跨天构造测试用例会把「未到」误判成「已到」

验证「时刻未到不应触发」时，若用 `now + timedelta(hours=3)` 构造目标时刻，
在 23:05 时它会跳到次日 02:05，而按「当天分钟数」比较，02:05 < 23:05 会被判成
「已过」。**这是用例构造的缺陷，不是实现缺陷**，但很容易被误当成 bug 去改实现。

排查手法：先打印实际比较的两个数值。

```python
target_minutes = plan["hour"] * 60 + plan["minute"]   # 02:05 -> 125
now_minutes = now.hour * 60 + now.minute              # 23:05 -> 1385
# 125 < 1385 -> 判定「已过」
```

### 附带需要知情的语义：错过就补跑

判据是「今天已过设定时刻 **且** 今天没清过」，因此进程在目标时刻之后才启动时会
立即补清一次。这是**有意为之**（保证「每天必清一次」），但副作用要写清楚：
把时刻设在 04:00，而 Bot 当天 23:00 才重启，重启后会马上清一次。

### 最终验证（服务器本机探针，五用例）

```
用例 1  未启用                -> 不清空，保持 (5, 2)         通过
用例 2  今天该时刻尚未到      -> 不清空                       通过
用例 2b 今天已过且今天没清过  -> 补跑一次，(0, 0)             通过
用例 3  已启用且时刻已到      -> 清空；永久记忆 1 / 好感度 1 未变 通过
用例 4  同一天再跑            -> 不重复清空，新写入 (3, 1) 保留  通过
用例 5  keep_days=7           -> 30 天前记录被裁掉，仅留「刚刚」  通过
```

线上实测：对话记忆 226 条、用户档案 92 个被清；同时断言永久记忆 14 条、
好感度 89 项、性格演化 7 条、心情 3 条**全部未变**。

## 二十一、评论与私信「串台」：永久记忆膨胀淹掉用户那句话

### 症状

用户报的原话是「不同评论和私信会串台」。表现有两类，主观上容易被归成同一件事：

1. 回 A 的话像在回 B —— 内容对不上来信；
2. 答非所问、把对方原话改写一遍当回答（「今天天气怎么样」→「今天天气怎么样呀？」）。

### 先证伪「串台」这个前提

第一反应是查会话隔离：`thread_id` 粒度、上下文是否跨会话复用、私信游标是否串号。查下来都不是。

| 检查项 | 结论 |
|--------|------|
| `thread_id` 粒度 | 私信为 `private:{mid}`，评论按楼层，**按会话正确隔离** |
| `build_memory_context` | 只读该会话的对话记忆，**无跨会话读取** |
| 私信 seqno 游标 | 每会话独立，**无串号** |
| 去重表 | 101 个会话 / `processed_keys=150`，符合定长表裁剪预期 |

真正把结论定下来的一步是**把线上真实 prompt 拉下来看**。写了个脚本在远端直接调 `build_memory_context` + 拼接逻辑，把最终 prompt 落盘：

```
整段 prompt 字符数：18030
```

而用户那句话占多少？**约 10 个字符，排在整段的最末尾。**
...
```python
PERMANENT_MEMORY_LIMIT = 40      # 最多 40 条
PERMANENT_MEMORY_INJECT = 40     # 最多注入 40 条
```

**条数完全约束不了单条长度**。14 条远没到 40 的上限，但它已经吃掉了 9375 字符——因为有人往池里塞了几条表情包清单，每条几百字。

### 为什么「前缀越重」看起来像「串台」

每条会话的 prompt 前缀（persona + 全部永久记忆）**完全相同**，只有最末尾那一句不同。前缀权重越高，末尾那句的相对影响力越低，两条不同私信生成出来的回复就越趋同。

```
会话 A prompt: [7253 字人格][9375 字记忆] ... [你说：你也是区]
会话 B prompt: [7253 字人格][9375 字记忆] ... [你说：今天天气怎么样]
                                  ↑ 16628 字符完全相同
```

模型在「一堆素材」里找话说，于是输出退化成素材风格的空话。**这不是上下文污染，是注意力分配问题**——但表面现象和串台一模一样，所以第一直觉会带偏排查方向。

### 修复：分层装填取代按条数平铺

新增 `_rule_tier(text)`，按规则头部特征分层：
...
def _rule_tier(text):
    ...
    if any(k in head for k in ("【身份】", "【人格", "行为原则", "【说话风格】")):
        return 0
    if any(k in head for k in ("禁止", "底线", "【安全", "抗越狱",
                               "冲突裁决", "优先级", "边界")):
        return 1
    if any(k in head for k in ("不懂", "不确定", "无法理解",
                               "理解用户", "今日心情", "短期状态")):
        return 2
    if "表情包" in head:
        return 3
...
`_build_permanent_block()` 按层装填，核心决策是**tier 0/1 无条件全量注入**：

```python
for tier in sorted(grouped):
    entries = sorted(grouped[tier], key=lambda p: -p[0])   # 同层内新的先保
    if tier in (0, 1):
        for _, text in entries:
            kept.add(text); core_chars += len(text) + 3     # 不看预算
        continue
    quota = max(0, budget - core_chars)                     # 预算只约束 2/3/9
    if tier == 3 and quota <= 0:
        continue
    used = 0
    for idx, text in entries:
        cost = len(text) + 3
        if used and used + cost > quota:                    # 首条无条件保留
            continue
        kept.add(text); used += cost; core_chars += cost
kept_list = [t for t in rules if t in kept]                 # 还原书写顺序
```

**为什么必须让 tier 0/1 绕过预算**：这一点是单测逼出来的，不是一开始就想到的。

最初的设计是「按占比给各层切配额」（45% / 40% / 15%）。单测直接挂掉——2500 字符按占比切完，tier 0 只得 1125 却要装 1318 字符、tier 1 只得 1000 却要装 2155 字符，两组都从「最旧那条」开始丢弃，结果**【身份】和【禁止复读与复述】双双出局**——而后者恰恰就是本次要修的规则。

按占比硬切配额这个思路的错处在于：它假设各层内容可以按比例缩水，但**人格底线是不可缩水的**——宁可挤爆预算也不能丢。改成 tier 0/1 无条件注入后，预算压到 10 字符时核心规则依然在位。

### 表情包池的摘要化

`_summarize_emoji_pool()` 把池清单压成「共约 N 条 + 每段前 6 个样例 + 使用约束」：

```
【可用表情包素材（索引摘要）】
- 共约 120 条，按主题分 5 类
- 「猫猫」类示例：喵喵叫、猫猫探头、猫猫疑惑、猫猫震惊、猫猫打滚、猫猫喝茶 …
- 使用约束：按语境挑选，不要一次列多个名字
```

实测单条 3160 → 338 字符，压缩率约 89%。**池子只需要知道「有哪些素材」，不需要把每个名字念一遍。**

### 用户消息独立成边界段

末尾追加三条硬约束，第一条是针对性最强的——直接点名「复述用户原话当回答」这个具体反模式，并给出正例：
...
- 对方提了具体请求（写诗、解释、推荐等）就实际完成它，不要只回「好的，我会
  尽力」这类空承诺。
```

### 排查手法：三条可复用的

**1. 症状名不等于根因名。** 用户说「串台」，先做的是**证伪串台**——把会话隔离逐项查一遍确认无误，再转向 prompt 本身。如果一上来就顺着「串台」修 `thread_id` 或游标，会白改一轮。

**2. prompt 要能落盘量化，不能靠想象。** 在远端直接调上下文构造函数、把最终 prompt 存文件，然后**统计各段字符数与占比**。18030 这个数字和「用户那句话只有 10 字符」这个对比，是整个定位过程里唯一真正决定性的事实。

**3. 用真实模型做前后对比表。** 光看 prompt 变短了不算验证，得拿修复前的失败用例重跑，逐条比对回复。本轮用了四条覆盖不同失败模式（复读 / 误判攻击 / 成品交付），单看一条容易漏。

### 副产品：Bot 业务日志不在 journal

排查中途一度以为 Bot 没有任何日志输出。实际是 `bilibili-bot.service` 里写了：

```ini
StandardOutput=append:/var/log/bilibili-bot.log
```

业务日志全在这个文件里，`journalctl -u bilibili-bot` 只能看到 systemd 自己的启停记录。**排查 Bot 行为必须 `tail /var/log/bilibili-bot.log`。**

## 二十二、复读与空承诺：从宣布意图到交付成品

### 症状

上一条修完，四条回归用例里三条彻底好，第四条还不行：

```
[来信] 帮我写首诗
[回复] '好的，我来帮你写一首诗。'      ← 只是一个承诺，没有诗
```

注意它**已经不是原来的「我会尽力」**了——措辞换了、语气更具体了，但**依然没写诗**。这种「看起来像修好了」的残留最容易漏掉，所以回归用例必须做结构化校验，不能目测。

### 根因：两条规则直接打架

```
规则 A（新加）：对方提了具体请求（写诗、解释、推荐等）就实际完成它。
规则 B（原有）：reply简短自然，一般15-40字，像B站真人回复，不要写得像作文。
```

一首四行五言诗本身就是 20-40 字，形态正是「像作文」。

模型的选择不是随机的——**规则 B 更靠后、更具体、带否决语（「不要写得像作文」）**，在提示词中权重更高。于是模型选择服从长度约束，用一个长度合规的承诺句同时满足两条规则的字面要求。

**这也解释了为什么「把上限调大到 200 字」是错的解法**：问题不在数字大小，在于「不要写得像作文」这个否决语——写诗必然触发它。只要否决语还在，调大上限只会让承诺句变长。

### 修复：让长度约束显式让位

两处对称修改，都在提示词层，无逻辑分支。

**（1）成品条款里显式声明豁免，并给出可复制的示范：**
...
示范里带了 **`\n` 的真实写法**。这点很关键：`reply` 是 JSON 字符串字段，分行必须用转义换行符。如果不示范，模型可能因为「JSON 里不好换行」而回避分行——给出可照抄的形态比讲道理有效。
...
两处都写是有意的：**规则冲突要在冲突双方各自的所在位置就近声明豁免**，只在一处声明，另一处的否决语依然会赢。

### 验证：结构化判定，而不是目测

判定脚本 `_repro_deliverable.py` 按请求类型做结构化校验：

| 类型 | 判定条件 |
|------|----------|
| `poem` | 有换行分行且 ≥3 个短句；或 ≥3 短句且总长 ≥20 字 |
| `list` | 有序号，或 ≥3 个书名号 / 引号包裹的条目 |
| `answer` | 人工判断语义匹配（不自动判） |
...
**为什么要自动判定**：「帮我写首诗」的失败回复「好的，我来帮你写一首诗」里含有「诗」字。人眼一扫觉得「提到了诗，应该算过」，实际上一个字都没写。结构化判定（要有分行、要 ≥3 短句）能挡住这类**形似而实不至**的通过。

### 排查手法提炼

**1. 「改善」不等于「修复」。** 失败输出从「我会尽力」变成「我来帮你写」时，最容易误判为已修好。判断标准必须是**产物是否存在**，而不是措辞是否变好。

**2. 规则冲突要量化到「谁更靠后、谁更具体、谁带否决语」。** 提示词里两条矛盾规则，模型不是随机的——它服从位置更靠后、表述更具体、带否定词的那条。定位冲突时按这三个维度排优先级，比反复调措辞有效。

**3. 给出可照抄的示范，胜过描述要求。** 写诗的示范里带了 `\n` 转义写法，模型一次就照做。只说「请注意分行」大概率无效——它不知道在 JSON 字段里该怎么分。
